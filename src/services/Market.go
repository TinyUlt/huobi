...
//------------------------------------------------------------------------------------------
// 交易API
func GetUstdData(coinId string , tradeType ,page int)models.USDTReturn{
	uSDTReturn := models.USDTReturn{}
	if tradeType ==1 {
		uSDTReturn.TradeType = "Sell"
	}else{
		uSDTReturn.TradeType = "Buy"
	}
	mapParams := make(map[string]string)
	mapParams["coinId"] = coinId
	mapParams["tradeType"]= fmt.Sprintf("%d", tradeType)
	mapParams["&currPage"] = fmt.Sprintf("%d", page)
	mapParams["merchant"] = "0"
	mapParams["online"] = "1"
	mapParams["range"] = "0"
...
func GetMarketPrice() (string, string, string){
	...
	ethPrice:=GetPriceByCoinId(uSDTReturn.Data,3)
	usdtPrice:=GetPriceByCoinId(uSDTReturn.Data,2)
	return btcPrice, ethPrice,usdtPrice
}
func GetPriceByCoinId(prices []models.USDTPriceData, id int)string{
